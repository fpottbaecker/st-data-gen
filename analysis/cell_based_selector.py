...
ST_FILE = "../../data/HCA_split/generated/harvard-donor-H4.st.h5ad"
TREE_DEPTH = 10
ITERATION_COUNT = 2
...
def evaluate_rmse(actuals, predicteds):
    ...
    print(f"RMSE: all={np.sqrt(np.mean(squared_errors))}, quartiles={np.quantile(np.sqrt(np.mean(squared_errors, axis=1)), [0.0, 0.25, 0.5, 0.75, 1.0])}")
